...
:: -- DeSOTA DEVELOPER Tool (Portable Python): targeted for models without Python

:: USER PATH
:: %~dp0 = C:\Users\[username]Desota\Portables\TinyPython\executables\Windows
for %%a in ("%~dp0\..\..\..\..\..") do set "user_path=%%~fa"
for %%a in ("%~dp0\..\..\..\..\..\..") do set "test_path=%%~fa"
for %%a in ("%UserProfile%\..") do set "test1_path=%%~fa"

:: Model VARS
set model_name=TinyPython
set model_path_basepath=Desota\Portables\%model_name%

:: - Model GIT
set model_release=https://github.com/desotaai/tinypython/archive/refs/tags/v0.0.0.zip
...
IF %debug_bool% EQU 1 GOTO noisy_conda
